...
export default function Home() {
  ...
  const [network, setNetwork] = useState<NetworkParams>(() => initializeNetwork());
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-12">
        <header className="flex flex-col gap-6 text-center md:text-left">
          <span className="mx-auto inline-flex items-center gap-2 rounded-full bg-white/60 px-4 py-2 text-sm font-medium text-sky-700 shadow-sm backdrop-blur-md md:mx-0">
            ...
            Realtime Reinforcement Learning Playground
          </span>
          <h1 className="text-4xl font-semibold text-slate-900 md:text-5xl">
            CartPole Visual Sandbox
          </h1>
          <p className="text-lg text-slate-600 md:max-w-2xl">
            Watch a lightweight CartPole environment balance itself using a
            simple heuristic policy. Toggle different control strategies, pause
            the action, or reset the scene to explore the dynamics of this RL
            classic.
          </p>
        </header>
          ...
          <aside className="flex flex-col gap-6">
            <div className="rounded-2xl border border-white/50 bg-white/80 p-6 shadow-sm">
              ...
              <div className="mt-4 space-y-3 text-sm">
                <MetricRow
                  label="Steps this episode"
                  value={metrics.steps.toString()}
                />
                <MetricRow
                  label="Reward this episode"
            ...
            <div className="rounded-2xl border border-white/50 bg-white/90 p-6 shadow-sm">
              ...
              <p className="mt-4 text-xs text-slate-500">
                Connections thicken and shift color as weights strengthen (blue)
                or weaken (rose). Node fill hints at the latest activations.
              </p>
            </div>
            <div className="rounded-2xl border border-white/40 bg-gradient-to-br from-sky-500/20 via-indigo-500/20 to-blue-600/40 p-6 text-sm text-slate-600 shadow-sm">
              ...
              <p className="mt-3">
                The CartPole environment balances a pole by applying left or
                right forces to a cart. Our heuristic controller uses pole angle
                and velocity to decide which direction to push. Switch to
                &quot;Random&quot; to see how quickly the pole falls without a sensible
                policy.
              </p>
            </div>
          </aside>
